...
<script type="text/javascript">
	...
	$(function () {
		...
		grid.addColumn({ label: '<spring:message code="contract.field.submit"/>', name: '<spring:message code="contract.field.send.date"/>', width: 100, sortable: false });
		...
		grid.setController("<c:url value='/contract/list/query' />");
	...
	function girdCmds(cellvalue, options, rowObject) {
		var contractor = rowObject.承辦人員;
		//移轉合約 
		if(rowObject.承接人 != '' & rowObject.承接人 != undefined){
		...
		if(rowObject.isNowUser == 'Y'){
			//簽核 && 編輯
			//當承辦人承接人為user或者為代理人且狀態為編輯中 
			if((contractor == userCname || rowObject.被代理申請人=='<c:out value="${principal}"/>') && rowObject.狀態 == '編輯中'){
        	...
        	if(contractor == userCname || rowObject.agentUserId == '<c:out value="${principal}"/>'){
        		if(rowObject.todo == '暫存' || rowObject.todo == '新建'){
        			cmd += '<button id="btnEdit" type="button" class="btn btn-sm btn-success" ' +
		 			'onclick="toView(\'' + options.rowId +'\', \'2\');">編輯</button>';
        		} else if(rowObject.todo == '作廢'){
        		} else {
       	    		cmd = '<button id="btnDetail" type="button" class="btn btn-sm btn-primary" ' +
    	    		'onclick="toView(\'' + options.rowId +'\', \'0\');">檢視</button>';
        		}
			...
			<div class="card-body">				
				...
				<form id="dataForm" name="dataForm" class="forms-sample" action="<c:url value='/contract/list/view' />" method="POST">
					...
					<div class="row">
						<div class="col-md-6">
							<div class="form-group row">
								<label data-require=true class="col-sm-2 col-form-label"><spring:message code="contract.field.submit"/>起</label>
								<div class="col-sm-9 ">
						...
						<div class="col-md-6">
							<div class="form-group row">
								<label data-require=true class="col-sm-2 col-form-label"><spring:message code="contract.field.submit"/>迄</label>
								<div class="col-sm-9">
