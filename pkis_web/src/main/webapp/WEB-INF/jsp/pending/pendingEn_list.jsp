...
<script type="text/javascript">
	...
	$(function () {
		...
		grid.addColumn({ label: '<spring:message code="contractEn.Tolist.Edit"/>', width:100, exportcol:false,sortable: false, formatter:girdCmds});
		grid.addColumn({ label: '<spring:message code="contractEn.Tolist.CheckStatus"/>', name: 'checkStatic', sortable: false , hidden: true});
		grid.addColumn({ label: '<spring:message code="contractEn.Tolist.CheckResult"/>', name: 'checkResult', width: 110, sortable: false});
		grid.addColumn({ label: '<spring:message code="contractEn.Tolist.ContractNumber"/>', name: 'applyNo', width: 110, sortable: false });
		grid.addColumn({ label: '<spring:message code="contractEn.Tolist.ContractFormat"/>', name: 'contractmodel', width: 70, sortable: false });
		grid.addColumn({ label: '<spring:message code="contractEn.Tolist.ContractName"/>', name: 'contracttype', width: 90, sortable: false });
		grid.addColumn({ label: '<spring:message code="contractEn.Tolist.SupplierTaxNumber"/>', name: 'suppliergui', width: 100, sortable: false });
		grid.addColumn({ label: '<spring:message code="contractEn.Tolist.SupplierNameCh"/>', name: 'supplierCname', width: 150, sortable: false });
		grid.addColumn({ label: '<spring:message code="contractEn.Tolist.Submitdate"/>', name: 'startDay', width: 100, sortable: false });
		grid.addColumn({ label: '<spring:message code="contractEn.Tolist.Status"/>', name: 'flowStatus', width: 90, sortable: false });
		grid.addColumn({ label: '<spring:message code="contractEn.Tolist.InChargeofPerson"/>', name: 'contractor', width: 70, sortable: false });
		grid.addColumn({ label: '<spring:message code="contractEn.Tolist.Department"/>', name: 'deptNo', width: 50, sortable: false });
	...
	function girdCmds(cellvalue, options, rowObject) {
        ...
        if(rowObject.flowStatus == '編輯中'){
			 cmd += '<button id="btnEdit" type="button" class="btn btn-sm btn-success" ' +
	 			'onclick="toDetailView(\'' + options.rowId +'\');">Edit</button>';
		 }else{
			 cmd += '<button id="btnFlow" type="button" class="btn btn-sm btn-success" ' +
	 			'onclick="toDetailView(\'' + options.rowId +'\');">Approval</button>';
		 }
