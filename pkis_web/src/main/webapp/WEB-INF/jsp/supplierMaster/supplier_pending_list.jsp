...
<script type="text/javascript">
	...
	$(function () {
		...
		grid.addColumn({ label: '<spring:message code="contract.field.no"/>', name: 'applyNo', width: 90, sortable: false });
		...
		grid.addColumn({ label: '<spring:message code="contract.field.contractor"/>', name: 'contractor', width: 90, sortable: false });
		grid.addColumn({ label: '<spring:message code="contract.field.section"/>', name: 'deptNo', width: 90, sortable: false });
		...
		grid.addColumn({ label: '<spring:message code="contract.field.check_static"/>', name: 'checkStatic', width: 90, sortable: false, hidden: true});
		grid.addColumn({ label: '<spring:message code="contract.field.check_result"/>', name: 'checkResult', width: 90, sortable: false, hidden: true});
		
		grid.setController("<c:url value='/supplier/pending/toDoList' />");		
	...
	<div class="col-12 grid-margin stretch-card">
		<div class="card">
			...
			<header class="card-title">
				<c:out value="${menuName}"/>(共 <span id="totalCnt">0</span> 筆)
			</header>
			</div>
			<form id="dataForm" name="dataForm" class="forms-sample" action="<c:url value='/supplier/pending/view' />" method="POST">
